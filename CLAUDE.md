...
python -m dftp.server --root /path/to/serve -v

# Run client
python -m dftp.client localhost --port 4433 --insecure
...
**Protocol** (`src/dftp/protocol.py`): Length-prefixed JSON frames (4-byte big-endian uint32 + JSON payload). Commands: LIST, PWD, CD, MKDIR, RMDIR, DELETE, GET, PUT, QUIT.
